...
DataTypes.ABSTRACT.prototype._toRawString = function() {
  if (this.key === 'DECIMAL') {
    return `${this.key}(${this.options.precision},${this.options.scale})`
  } else if (this.options && this.options.length) {
    return `${this.key}(${this.options.length})`
  } else {
    return this.key
  }
}
class Integrate {
  ...
  static isEqual(
    attr1: ArrayDataTypeOptions<AbstractDataType>,
    attr2: ArrayDataTypeOptions<AbstractDataType>
  ): boolean {
...
`
  ...
  static generateModelObjectLiteralData(
    modelName: string,
    model: ModelAttributes,
    dbComment?: dbCommentObj
  ) {
    const upperModelName = _.upperFirst(modelName)
    const fields = Object.keys(model)
    let out = ''
